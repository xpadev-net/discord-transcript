...
- `recording_duration_is_estimate` is a meeting metadata boolean, initially false, set true only when an emergency abort cannot compute exact elapsed recording duration and must use the last-known elapsed duration.
...
- Period usage increments when a meeting reaches any terminal state (`posted`, `failed`, or `aborted`) with a known recording duration. For `aborted` meetings, `recording_duration_seconds` must be computed and stored in the same abort transaction when the meeting had previously transitioned to `recording`; if the abort is an emergency path where duration is unavailable, record the last-known elapsed duration and set `recording_duration_is_estimate = true`.
- `recording_minutes` usage is allocated to the quota period that contains `recording_started_at`, even if the meeting reaches a terminal state in a later period. The Effective Settings Snapshot must store `recording_period_start` and `recording_period_end` so terminal-state accounting writes the event to the original period; the initial contract does not split one recording across periods.
...
- When a finite `storage_bytes` soft quota is enforced and the gauge is stale, allow the operation, enqueue or trigger a gauge rebuild, and record an observable stale-gauge quota event using the last known `current_value` when present or `0` when no row exists. For that event, use `source_type = stale_gauge`, set `source_id` to the tenant artifact mutation sequence assigned to the storage-increasing operation when one exists, otherwise to a durable rebuild request id generated when enqueueing the rebuild, and set `amount_over = max(0, observed_value - limit_value)`.
- When a finite `storage_bytes` soft quota is enforced and the gauge is fresh, allow the storage-increasing operation, update the gauge in the same transaction, and if the resulting `current_value` exceeds the finite limit, record an observable quota violation event with `source_type = artifact_mutation`, `source_id` equal to the tenant artifact mutation sequence assigned to that storage change, `observed_value` equal to the resulting gauge value, and `amount_over = max(0, observed_value - limit_value)`.
...
- `period_end`: nullable.
- `source_type`: one of `meeting`, `asr_attempt`, `summary_invocation`, `download_session`, `artifact_mutation`, or `stale_gauge`.
- `source_id`
- `observed_at`
- `created_at`
...
- In the initial one-guild-per-tenant phase, `guild_id` is required for every quota violation event and is resolved from the active tenant-guild binding at `observed_at`, including tenant-level `storage_bytes` gauge events. For post-hoc units such as `recording_minutes`, `guild_id` is sourced from the Effective Settings Snapshot captured at recording start rather than the active binding at `observed_at`, consistent with the Period Counter Usage Event rule. `guild_id` may be null only for a future tenant-level or organization-level event that does not correspond to one active guild.
...
- A `closed` tenant keeps historical data ownership for retention, audit, and read-only access, but cannot receive new guild bindings or plan assignments and must fail all new resource-consuming operations. Cleanup and delete operations, including TTL cleanup jobs and explicit artifact deletions, may proceed when otherwise authorized because they reduce or preserve retained data.
...
- `recording_period_start`: inclusive period boundary derived from `recording_started_at`, `plan_assignment_id`, and the assignment's `period_anchor` when the snapshot is created.
- `recording_period_end`: exclusive period boundary paired with `recording_period_start`.
- `resolved_at`
...
- `source_versions`: metadata for the env, tenant, and guild layers used to resolve the snapshot, shaped as `{ "env": { "version": 1, "hash": "<lowercase-hex-sha256>" }, "tenant": { "id": "<tenant_id>", "version": 3 }, "guild": { "id": "<guild_id>", "version": 7 } }` where `env.version` is a JSON integer, tenant and guild `version` fields are JSON integers when a settings row exists and JSON null when no row exists, and `id`/`hash` fields are JSON strings. `env.version` is the environment-settings schema version, initially `1`; increment it when snapshotted env-default fields are added, removed, or renamed, or when an existing snapshotted field's type or allowed values change. `env.hash` is calculated as:
  ...
  - Required integer and string env normalization: participating required integer fields and `bot_token_source` must be captured from raw environment strings before typed parsing. Integer env strings must be present, base-10 decimal integer notation with no surrounding whitespace, no decimal point, and no scientific notation, and must satisfy the application's existing range validation. `bot_token_source` must be present as a non-empty allowed source marker string. Implementations must validate the presence and format of these required env strings at process startup; typed code defaults are not sufficient for hash computation.
  - Boolean env normalization: participating boolean fields must be captured from raw environment strings before typed parsing and must be exactly `true` or `false` in lowercase with no surrounding whitespace. The hash input serializes those values as JSON booleans. If the raw string is unavailable, snapshot hash generation fails; a typed code default is not sufficient for hash computation. Any other boolean env string, including `1`, `0`, `yes`, `no`, `on`, `off`, `True`, or `False`, makes snapshot hash generation fail. Implementations must validate the presence and format of all participating boolean env strings at process startup.
...
- The intended uniqueness constraint is `meeting_id`.
...
- Audit the five boolean snapshot environment variables before enabling startup validation: `WHISPER_LANGUAGE_EXPLICIT`, `WHISPER_VAD`, `WHISPER_SUPPRESS_NON_SPEECH`, `WHISPER_RESAMPLE_TO_16K`, and `SUMMARY_ENABLED` must be configured as lowercase `true` or `false`, not `1`, `0`, `yes`, `no`, `on`, `off`, `True`, or `False`.
- Add `BOT_TOKEN_SOURCE` and the five required integer snapshot environment variables to local development, CI, and deployment configuration before enabling startup validation. The integer variables are `WHISPER_BEAM_SIZE`, `AUTO_STOP_GRACE_SECONDS`, `RETENTION_RAW_AUDIO_TTL_DAYS`, `RETENTION_TRANSCRIPT_TTL_DAYS`, and `RETENTION_SUMMARY_TTL_DAYS`; code defaults are not sufficient once snapshot hash validation is enabled.
