...
fn load_final_transcript_segments<E: SqlExecutor>(
    ...
    let rows = executor.query_rows(
        "SELECT speaker_id, start_ms, end_ms, text, confidence, is_noisy, source \
         ...
         ORDER BY start_ms, end_ms, source, substring(id from '-t-([0-9]+)$')::INTEGER NULLS LAST, id",
        &[meeting_id.to_owned()],
    )?;
    let mut segments = rows
        .iter()
...
impl ScaffoldHandler {
    ...
    async fn process_enqueued_summary_job(
        ...
        let (final_segments, live_segments, completed_live_chunks) = {
            let mut service = self.service.lock().await;
            let final_segments = match load_final_transcript_segments(
                &mut service.store.executor,
                &meeting.id,
            ) {
                Ok(value) => value,
            ...
            let completed_live_chunks = if final_segments.is_empty() && live_segments.is_empty() {
                Vec::new()
            } else {
            ...
            (final_segments, live_segments, completed_live_chunks)
        };
